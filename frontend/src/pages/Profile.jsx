...
import { User, Settings, MessageSquare, Heart, Users, Award, Clock, Bell } from 'lucide-react';
...
function Profile() {
  ...
  const fetchUserPosts = async () => {
    try {
      ...
      // Filtrer les posts créés par l'utilisateur actuel
      if (user && response.data && response.data.data) {
        const posts = response.data.data.filter(post => {
          return post.attributes && post.attributes.createdBy && 
                 post.attributes.createdBy.data && 
                 post.attributes.createdBy.data.id === user.id;
        });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {userPosts.map(post => (
                <div key={post.id} className="bg-[#18181b] rounded-lg p-4 hover:bg-[#3f3f46] transition-colors cursor-pointer">
                  <h4 className="font-medium">{post.attributes.title || 'Sans titre'}</h4>
                  <p className="text-[#a1a1aa] text-sm truncate">{post.attributes.description || 'Pas de description'}</p>
                  <div className="flex justify-between items-center mt-3 text-[#a1a1aa] text-sm">
                    <span className="flex items-center">
                      <Heart size={14} className="mr-1" /> 0
                    </span>
                    <span className="flex items-center">
